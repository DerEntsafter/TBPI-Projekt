...
public:
  ...
  bool performOffsetCalibration(uint32_t target_distance_mm) {
    ...
    FixPoint1616_t distance_1616 = (FixPoint1616_t)(target_distance_mm << 16);

    VL53L0X_Error status = VL53L0X_PerformOffsetCalibration(&vl53l0x_dev, distance_1616, &offsetMicroMeter);
    if (status != VL53L0X_ERROR_NONE) {
      print_pal_error(status, "VL53L0X_PerformOffsetCalibration");
      return false;
    }
    ESP_LOGI(TAG, "Offset Calibration Successful. Offset: %ld um", offsetMicroMeter);
    return true;
  }

  bool performXTalkCalibration(uint32_t target_distance_mm) {
    FixPoint1616_t xTalkRateMegaCps = 0;
    // The API expects distance as FixPoint1616_t (shifted by 16 bits)
    FixPoint1616_t distance_1616 = (FixPoint1616_t)(target_distance_mm << 16);

    VL53L0X_Error status = VL53L0X_PerformXTalkCalibration(&vl53l0x_dev, distance_1616, &xTalkRateMegaCps);
    if (status != VL53L0X_ERROR_NONE) {
      print_pal_error(status, "VL53L0X_PerformXTalkCalibration");
      return false;
    }
    ESP_LOGI(TAG, "XTalk Calibration Successful. Rate: %ld", xTalkRateMegaCps);
    return true;
  }
